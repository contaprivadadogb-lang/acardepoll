...
function drawPocket(p){
  // sombra de entrada
  const grad = ctx.createRadialGradient(p.x,p.y,PR*0.3,p.x,p.y,PR*1.15);
  grad.addColorStop(0,'#000'); grad.addColorStop(0.75,'#000'); grad.addColorStop(1,'rgba(60,35,18,0)');
  ctx.beginPath(); ctx.fillStyle=grad; ctx.arc(p.x,p.y,PR*1.15,0,7); ctx.fill();
  ctx.beginPath(); ctx.strokeStyle='rgba(255,214,140,.35)'; ctx.lineWidth=2;
  ctx.arc(p.x,p.y,PR,0,7); ctx.stroke();
}
function draw() {
  if (!feltGrad) buildGradients();
  // fundo de madeira (rail) com friso dourado
  ctx.fillStyle = woodGrad; ctx.fillRect(0,0,W,H);
  ctx.strokeStyle = 'rgba(230,190,110,.55)'; ctx.lineWidth = 3;
  ctx.strokeRect(RAIL*0.55, RAIL*0.55, W-RAIL*1.1, H-RAIL*1.1);
  // borracha (cushion) escura entre madeira e feltro
  ctx.fillStyle = '#0d3550';
  ctx.fillRect(RAIL, RAIL, W-RAIL*2, H-RAIL*2);
  // feltro azul
  const fx = RAIL+CUSH, fy = RAIL+CUSH, fw = W-2*(RAIL+CUSH), fh = H-2*(RAIL+CUSH);
  ctx.fillStyle = feltGrad; ctx.fillRect(fx, fy, fw, fh);
